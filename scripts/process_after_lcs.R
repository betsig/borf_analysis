...
org="yeast"
for(org in c("human", "zebrafish", 'aradopsis', "yeast")){
    ...
    for(do_set in c("both_strands", "pos_strand")){
        ...
        load(paste0("data/",org, "/LCS/lcs_data_summary.rdata"))
        ...
        if(do_set == "both_strands"){
            write.table(trinity_translated, paste0('data/', org, "/trinity_translated_both_strand_partials.txt"), sep='\t', quote=F, row.names = F)
        }else{
            write.table(trinity_translated, paste0('data/', org, "/trinity_translated_partials.txt"), sep='\t', quote=F, row.names = F)
        }
        ...
        if(do_set == "both_strands"){
            write.table(trinity_translated_np, paste0('data/', org, "/trinity_translated_both_strands.txt"), quote=F, row.names = F, sep='\t')
        }else{
            write.table(trinity_translated_np, paste0('data/', org, "/trinity_translated.txt"), quote=F, row.names = F, sep='\t')
        }
    }
}


for(org in c("human", "zebrafish")){
    load(paste0("data/", org, "/processed_ensembl.Rdata"))

    tt = fread(paste0('data/', org, "/trinity_translated.txt"), data.table = F)
    ...
    write.table(tt, paste0('data/', org, "/trinity_translated.txt"), quote=F, row.names = F, sep='\t')
    ...
    tt$ens_5utr_len = utr_annot$end[utr5_index][tt_index] - utr_annot$start[utr5_index][tt_index]
    write.table(tt, paste0('data/', org, "/trinity_translated_both_strands.txt"), quote=F, row.names = F, sep='\t')
