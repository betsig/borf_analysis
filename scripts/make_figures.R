...
dev.off()
 ...
 for(org in c('yeast',"human", "arabidopsis", 'zebrafish')){
   ...
   # tt = fread(paste0('data/', org, "/trinity_translated_both_strands.txt"), data.table = F)
   #
   # genemarks_results =  read_lst(paste0("data/", org, "/genemarks/Trinity.fasta.lst"))
   # genemarks_seq = halpme::read_fasta2df(paste0("data/", org, "/genemarks/Trinity.fasta.faa"))
   # genemarks_seq$seq_id = strv_split(genemarks_seq$seq_id, "\t", 1)
   #
   # genemarks_results = right_join(genemarks_results, genemarks_seq, by='seq_id')
   #
   # genemarks_results$transcript_id = strv_split(genemarks_results$seq_id, "[ ]", 1)
   # genemarks_results = genemarks_results[genemarks_results$transcript_id %in% tt$subject,]
   # genemarks_results = cbind(genemarks_results, tt[match(genemarks_results$transcript_id, tt$subject),])
   #
   # genemarks_results = genemarks_results[genemarks_results$read_counts >=100,]
   #
   # genemarks_seq_out = genemarks_results[,c("seq_id", "seq")]
   # genemarks_seq_out$seq_id = paste0(">", genemarks_seq_out$seq_id)
   # write.table(genemarks_seq_out, file=paste0("data/", org, "/genemarks/", org, "_genemarks_expressed.pep"), quote=F,row.names = F,col.names = F,sep="\n")
     ...
     # borf_seq_out$orf_id = paste0(">", borf_seq_out$orf_id)
 ...
 for(org in c("human", "arabidopsis", 'zebrafish', 'yeast')){
   ...
   if(file.exists(paste0("data/", org, "/borf/blastp_borf.txt"))){
     borf_blast = fread(paste0("data/", org, "/borf/blastp_borf.txt"), data.table = F)
   }else{
   ...
   transdecoder_results_pep = halpme::read_fasta2df(paste0("data/", org, "/Trinity.fasta.transdecoder.singleline.pep"))
   transdecoder_results_pep$orf_id = strv_split(transdecoder_results_pep$seq_id, "[ ]", 1)
   transdecoder_results_pep$transcript_id = strv_split(transdecoder_results_pep$orf_id, "[.]p", 1)
   transdecoder_results_pep$strand = strv_split2(transdecoder_results_pep$seq_id, "[(]", "[)]")

   locs = strv_split(transdecoder_results_pep$seq_id, "[ ]", 8)
   transdecoder_results_pep$start_site_nt = as.numeric(strv_split2(locs, "[:]", "[-]"))
   transdecoder_results_pep$stop_site_nt = as.numeric(strv_split2(locs, "[-]", "[(]"))
   transdecoder_results_pep$orf_length_aa = as.numeric(strv_split2(transdecoder_results_pep$seq_id, "len[:]", "[ ]"))
   transdecoder_results_pep$orf_length_aa = nchar(gsub("[*]", "", transdecoder_results_pep$seq))
   transdecoder_results_pep$orf_class = strv_split2(transdecoder_results_pep$seq_id, "type[:]", "[ ]")

   transdecoder_results = transdecoder_results_pep
   transdecoder_results$org =org
   ...
   #### GENEMARKS-ST
   genemarks_results = read_lst(paste0("data/", org, "/genemarks/Trinity.fasta.lst"))
   #genemarks_results$orf_id = strv_split(genemarks_results$seq_id, "[ ]", 1)
   genemarks_results$transcript_id = strv_split(genemarks_results$seq_id, "[ ]", 1)

   genemarks_results$start_site_nt = genemarks_results$leftend
   genemarks_results$start_site_nt[grep("<", genemarks_results$start_site_nt)] = 1
   genemarks_results$start_site_nt = as.numeric(genemarks_results$start_site_nt)

   genemarks_results$stop_site_nt = as.numeric(gsub(">", "", genemarks_results$rightend))
   genemarks_results$orf_length_aa = as.numeric(genemarks_results$gene_length) / 3

   genemarks_results = genemarks_results[genemarks_results$transcript_id %in% tt$subject,]
   genemarks_results = arrange(genemarks_results, desc(orf_length_aa))
   genemarks_results$org =org


   gm_blast = fread(paste0("data/", org, "/genemarks/genemarks_blast.txt.gz"), data.table = F)


   colnames(gm_blast) = c("query", "subject", "p_ident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore","sframe", "qframe")
   gm_blast$subject_transcript_id = ensembl_peps$transcript[match(gm_blast$subject, strv_split(ensembl_peps$seq_id, "[ ]", 1))]
   gm_blast$subject_gene_id = ensembl_peps$gene_id[match(gm_blast$subject, strv_split(ensembl_peps$seq_id, "[ ]", 1))]

   genemarks_results$base_hit = tt$query[match(genemarks_results$transcript_id, tt$subject)]
   genemarks_results$hit_blastp = gm_blast$subject_transcript_id[match(genemarks_results$transcript_id, gm_blast$query)]
   genemarks_results$base_in_blastp = ifelse(paste0(genemarks_results$transcript_id, genemarks_results$base_hit) %in% paste0(gm_blast$query, gm_blast$subject_transcript_id), 1, 0)

   genemarks_results$base_hit_gene = ensembl_peps$gene_id[match(genemarks_results$base_hit, ensembl_peps$transcript)]
   genemarks_results$base_in_blastp_gene = ifelse(paste0(genemarks_results$transcript_id, genemarks_results$base_hit_gene) %in% paste0(gm_blast$query, gm_blast$subject_gene_id), 1, 0)


   colnames(gm_blast) = paste0("blastp_gm_",colnames(gm_blast))
   genemarks_results = cbind(genemarks_results, gm_blast[match(genemarks_results$transcript_id, gm_blast$blastp_gm_query),])

   genemarks_results_add = cbind(genemarks_results, tt[match(genemarks_results$transcript_id, tt$subject),])

   gm_all = rbind(gm_all, genemarks_results_add)
   genemarks_results$org =NULL

   # not req. 99AA orfs are all incomplete at at least one end.
   #genemarks_results = genemarks_results[genemarks_results$orf_length_aa >=100,]
   #genemarks_results = genemarks_results[genemarks_results$strand == "+",]
   genemarks_results = arrange(genemarks_results, desc(orf_length_aa))
   genemarks_results = genemarks_results[!duplicated(genemarks_results$transcript_id),]
   #### FIN
   ...
   colnames(genemarks_results) = paste0(colnames(genemarks_results), "_genemarks")
   #tt = left_join(tt, transdecoder_results[,], by=c('subject'='transcript_id_transdecoder'))
   tt = cbind(tt, genemarks_results[match(tt$subject, genemarks_results$transcript_id),])
 ...
 gm_all$organism = "h. sapiens"
 gm_all$organism[gm_all$org == "zebrafish"] = "d. rerio"
 gm_all$organism[gm_all$org == "arabidopsis"] = "a. thaliana"
 gm_all$organism[gm_all$org == "yeast"] = "s. cerevisiae"
 ...
 td_all_exp = td_all[which(td_all$read_counts>= 100),]
 ...
 ####
 gm_all = arrange(gm_all, org, desc(orf_length_aa))

 gm_all_exp = gm_all[gm_all$read_counts>= 100,]
 gm_all_exp$longest_orf = ifelse(duplicated(paste0(gm_all_exp$org, gm_all_exp$transcript_id)), "Shorter ORF", "Longest ORF")
 gm_all_exp$longer_short_blastpmatch = paste0(gm_all_exp$longest_orf, " - ", ifelse(gm_all_exp$base_in_blastp == 1 | gm_all_exp$base_in_blastp_gene == 1, "Correct BlastP match", "Incorrect / no match"))
 gm_all_exp$base_in_blastp_gene_no = ifelse(gm_all_exp$base_in_blastp_gene == 1, "correct hit", "incorrect hit")
 gm_all_exp$base_in_blastp_gene_no[which(is.na(gm_all_exp$hit_blastp))] = "no hit"

 genemarks_table = as.data.frame(table(gm_all_exp$organism))
 colnames(genemarks_table)= c("Species", "total_orfs")
 genemarks_table$total_transcripts = as.data.frame(table(gm_all_exp$organism[!duplicated(paste0(gm_all_exp$organism, gm_all_exp$transcript_id))]))[,2]

 # multiple ORFs per tx
 genemarks_table = gm_all_exp %>% filter(longest_orf == "Shorter ORF") %>% filter(!duplicated(paste0(org, transcript_id))) %>% select(organism) %>% table() %>% as.data.frame() %>%
   rename(c('.'='Species', 'Freq'='total_transcripts_multi_orf')) %>% left_join(genemarks_table,.,by="Species")
 # txs with longest orf on - strand
 genemarks_table = gm_all_exp %>% filter(longest_orf == "Longest ORF" & strand=="-") %>% select(organism) %>% table() %>% as.data.frame() %>%
   rename(c('.'='Species', 'Freq'='longest_ORF_neg')) %>% left_join(genemarks_table,.,by="Species")
 # txs with incorrect longest orf on - strand
 genemarks_table = gm_all_exp %>% filter(longest_orf == "Longest ORF" & strand=="-"& base_in_blastp_gene_no != "correct hit") %>% select(organism) %>% table() %>% as.data.frame() %>%
   rename(c('.'='Species', 'Freq'='longest_ORF_neg_incorrect')) %>% left_join(genemarks_table,.,by="Species")

 genemarks_table$incorrect_neg_percent = (genemarks_table$longest_ORF_neg_incorrect / genemarks_table$total_transcripts)*100



 # multiple ORFs per tx
 genemarks_table = gm_all_exp %>% filter(base_in_blastp_gene_no == "incorrect hit") %>% select(organism) %>% table() %>% as.data.frame() %>%
   rename(c('.'='Species', 'Freq'='false_pos_hits')) %>% left_join(genemarks_table,.,by="Species")
 genemarks_table = gm_all_exp %>% filter(base_in_blastp_gene_no == "no hit") %>% select(organism) %>% table() %>% as.data.frame() %>%
   rename(c('.'='Species', 'Freq'='no_hits')) %>% left_join(genemarks_table,.,by="Species")

 genemarks_table$increase_blast_search = genemarks_table$total_orfs / genemarks_table$total_transcripts
 genemarks_table$false_pos_percent = (genemarks_table$false_pos_hits / genemarks_table$total_orfs)*100
 genemarks_table$no_hit_percent = (genemarks_table$no_hits / genemarks_table$total_orfs)*100

 genemarks_table.longest = genemarks_table[,c(1,3,5,6,7)]
 write_csv(genemarks_table.longest, "data/table_s_genemarks_longest.csv")
 genemarks_table.all = genemarks_table[,c(1,3,2,4,8,9,11,12,10)]
 write_csv(genemarks_table.all, "data/table_s_genemarks_all.csv")


 table(gm_all_exp$organism)
 table(gm_all_exp$organism[!duplicated(paste0(gm_all_exp$transcript_id, gm_all_exp$organism))])

 gm_all_shorter = gm_all_exp[gm_all_exp$longest_orf == "Shorter ORF",]
 gm_all_shorter = gm_all_shorter[!duplicated(paste0(gm_all_shorter$transcript_id, gm_all_shorter$organism)),]
 table(gm_all_shorter$organism)

 gm_strand_plot_data = as.data.frame(table(gm_all_exp$longer_short_blastpmatch, gm_all_exp$strand, gm_all_exp$organism))

 gm_strand_plot_data$Freq[gm_strand_plot_data$Var2 == "-"] = (gm_strand_plot_data$Freq *-1)[gm_strand_plot_data$Var2 == "-"]

 gm_strand_plot_data %>% filter(Var1 == "Longest ORF - Incorrect / no match") %>%
   filter(Var2 == "-")

 aggregate(Freq ~ Var3, gm_strand_plot_data, function(x) sum(abs(x)))

 # longest only - incorrect gene match
 longest_match = as.data.frame(table(gm_all_exp$base_in_blastp_gene[gm_all_exp$longest_orf == "Longest ORF"],
                                     gm_all_exp$strand[gm_all_exp$longest_orf == "Longest ORF"],
                                     gm_all_exp$organism[gm_all_exp$longest_orf == "Longest ORF"]))
 # incorrect/no gene match
 gm_all_exp$base_in_blastp_gene_no = ifelse(gm_all_exp$base_in_blastp_gene == 1, "correct hit", "incorrect hit")
 gm_all_exp$base_in_blastp_gene_no[which(is.na(gm_all_exp$hit_blastp))] = "no hit"

 table(gm_all_exp$organism[which(gm_all_exp$longest_orf == "Longest ORF" &
                                   gm_all_exp$strand =="-")])

 as.data.frame(table(gm_all_exp$base_in_blastp_gene_no== 'correct hit',
                     gm_all_exp$organism))

 all_match = as.data.frame(table(gm_all_exp$base_in_blastp_gene_no,
                                 gm_all_exp$strand,
                                 gm_all_exp$organism))

 corr_incor_no = as.data.frame(table(gm_all_exp$base_in_blastp_gene_no,
                                     gm_all_exp$strand, gm_all_exp$organism))


 table(gm_all_exp$base_class[which(gm_all_exp$base_class %in% c("complete", "incomplete_5prime") & gm_all_exp$base_class_tag == "full_match")],
       gm_all_exp$orf_class[which(gm_all_exp$base_class %in% c("complete", "incomplete_5prime") & gm_all_exp$base_class_tag == "full_match")],
       gm_all_exp$trinity_has_upstream_stop[which(gm_all_exp$base_class %in% c("complete", "incomplete_5prime") & gm_all_exp$base_class_tag == "full_match")])

 table(td_all_exp$base_class[which(td_all_exp$base_class %in% c("complete", "incomplete_5prime") & td_all_exp$base_class_tag == "full_match")],
       td_all_exp$orf_class[which(td_all_exp$base_class %in% c("complete", "incomplete_5prime") & td_all_exp$base_class_tag == "full_match")],
       td_all_exp$trinity_has_upstream_stop[which(td_all_exp$base_class %in% c("complete", "incomplete_5prime") & td_all_exp$base_class_tag == "full_match")])


 ### correct start_site comparisons
 keep = which(tt_all_exp$base_class %in% c("incomplete_5prime", "complete") & tt_all_exp$base_class_tag == "full_match")

 length(which(tt_all_exp$orf_class_borf[keep] == tt_all_exp$base_class[keep] & tt_all_exp$base_in_blastp_borf[keep] == 1))/ length(keep)
 length(which(tt_all_exp$orf_class_transdecoder[keep] == tt_all_exp$base_class[keep]& tt_all_exp$base_in_blastp_transdecoder[keep] == 1))/ length(keep)
 length(which(tt_all_exp$orf_class_genemarks[keep] == tt_all_exp$base_class[keep]& tt_all_exp$base_in_blastp_genemarks[keep] == 1))/ length(keep)
 ...
 tt_all = tt_all[keephigh,]

 tt_all$same_as_base = ifelse(tt_all$base_class == tt_all$orf_class_transdecoder &
                                tt_all$base_class == tt_all$orf_class_borf &
                                tt_all$base_class == tt_all$orf_class_genemarks &
                                tt_all$base_in_blastp_borf == 1 &
                                tt_all$base_in_blastp_transdecoder== 1 &
                                tt_all$base_in_blastp_genemarks == 1, "same", "no")

 td_correct = tt_all$base_class==tt_all$orf_class_transdecoder & tt_all$base_in_blastp_transdecoder== 1
 borf_correct = tt_all$base_class==tt_all$orf_class_borf & tt_all$base_in_blastp_borf== 1
 gm_correct = tt_all$base_class==tt_all$orf_class_genemarks & tt_all$base_in_blastp_genemarks== 1

 td_incorrect = tt_all$base_class!=tt_all$orf_class_transdecoder | tt_all$base_in_blastp_transdecoder!= 1
 borf_incorrect = tt_all$base_class!=tt_all$orf_class_borf | tt_all$base_in_blastp_borf!= 1
 gm_incorrect = tt_all$base_class!=tt_all$orf_class_genemarks | tt_all$base_in_blastp_genemarks!= 1


 tt_all$same_as_base[which(td_correct & borf_incorrect & gm_incorrect)] = "td_correct"
 tt_all$same_as_base[which(td_incorrect & borf_correct & gm_incorrect)] = "borf_correct"
 tt_all$same_as_base[which(td_incorrect & borf_incorrect & gm_correct)] = "gm_correct"

 tt_all$same_as_base[which(td_correct & borf_correct & gm_incorrect)] = "borf_and_td_correct"
 tt_all$same_as_base[which(td_incorrect & borf_correct & gm_correct)] = "borf_and_gm_correct"
 tt_all$same_as_base[which(td_correct & borf_incorrect & gm_correct)] = "gm_and_td_correct"

 tt_all$same_as_base[which(td_incorrect & borf_incorrect & gm_incorrect)] = "all_incorrect"

 tt_all$same_as_base %>% table()

 tt_all$same_as_base_c = as.character(tt_all$same_as_base)

 tt_all$same_as_base_c[tt_all$same_as_base_c == "same"] = "All"
 ...
 tt_all$same_as_base_c[tt_all$same_as_base_c == "gm_correct"] = "Genemarks"

 tt_all$same_as_base_c[tt_all$same_as_base_c == "borf_and_gm_correct"] = "Borf and Genemarks"
 tt_all$same_as_base_c[tt_all$same_as_base_c == "borf_and_td_correct"] = "Borf and Transdecoder"
 tt_all$same_as_base_c[tt_all$same_as_base_c == "gm_and_td_correct"] = "Genemarks and Transdecoder"

 tt_all$same_as_base_c[tt_all$same_as_base_c == "all_incorrect" | tt_all$same_as_base_c == "no" ] = "None"

 tt_all$same_as_base_c = factor(tt_all$same_as_base_c, levels = rev(c("All",
                                                                      "Genemarks and Transdecoder",
                                                                      "Borf and Transdecoder",
                                                                      "Borf and Genemarks",
                                                                      "Transdecoder", "Genemarks", "Borf", "None")))
 ...
 class_pal = c(at_color_pals$hex_value[at_color_pals$colour == "N" & at_color_pals$intensity ==inten],
 ...
 plot_corrincorr = ggplot(tt_all[!is.na(tt_all$same_as_base_c),], aes(x=base_class, fill=same_as_base_c)) + geom_bar() +
   scale_fill_manual(values=rev(class_pal), "Predicted ORF blasts\nto correct gene")  +
   theme_figure +
 ...
 # method accuracy
 method_pal = c(at_color_pals$hex_value[at_color_pals$colour == "T" & at_color_pals$intensity ==inten],
                at_color_pals$hex_value[at_color_pals$colour == "G" & at_color_pals$intensity ==inten],
                at_color_pals$hex_value[at_color_pals$colour == "Y" & at_color_pals$intensity ==inten])


 totals = table(tt_all$same_as_base, tt_all$organism) %>% as.data.frame()
 total_ts = aggregate(Freq ~ Var2, totals, sum)
 total_ts$borf_correct = aggregate(Freq ~ Var2, totals[totals$Var1 %in% c("same", "borf_correct", "borf_and_gm_correct", "borf_and_td_correct"),], sum)[,2]
 total_ts$td_correct = aggregate(Freq ~ Var2, totals[totals$Var1 %in% c("same", "td_correct", "borf_and_td_correct", "gm_and_td_correct"),], sum)[,2]
 total_ts$gm_correct = aggregate(Freq ~ Var2, totals[totals$Var1 %in% c("same", "gm_correct", "borf_and_gm_correct", "gm_and_td_correct"),], sum)[,2]

 total_ts$borf_acc = total_ts$borf_correct / total_ts$Freq
 total_ts$td_acc = total_ts$td_correct / total_ts$Freq
 total_ts$gm_acc = total_ts$gm_correct / total_ts$Freq

 total_ts_long = data.frame(organism = rep(total_ts$Var2, 3),
                            total_transcripts = rep(total_ts$Freq, 3),
                            correct_n = c(total_ts$borf_correct, total_ts$gm_correct, total_ts$td_correct),
                            accuracy = c(total_ts$borf_acc, total_ts$gm_acc, total_ts$td_acc),
                            method = rep(c("Borf", "Genemarks", "Transdecoder"), each=4))

 plot_method_accuracy = ggplot(total_ts_long, aes(x=organism, y=accuracy, col=method)) +
   geom_point() +
   theme_figure +
   scale_color_manual(values = rev(method_pal), "Method") +
 ...
 plot_runtime =
   ...
   geom_point() +  scale_color_manual(values = rev(method_pal), "Method")+
