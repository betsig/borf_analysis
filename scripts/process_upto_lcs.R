...
for(org in (c('human', 'yeast', 'arabidopsis', 'zebrafish'))){
  ...
  #coding_potential = fread("data/yeast/coding_potential/Trinity_cpc2_output.txt", data.table = F)
  
  # kallisto read counts
  kallisto = fread(paste0("data/", org, "/kallisto/kallisto_est_counts.tsv"),data.table = F, fill=T, sep='\t')
  ...
  write.table(unique(c(e2t_multi_gene_match$subject, e2t_multi_gene_match_pos$subject)), 
              file=paste0("data/", org, "/blastx_filter_trinity_ids.txt"), 
